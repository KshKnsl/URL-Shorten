...
// 🔹 Global variable instead of file
let globalUrl = "";
...
function generateshortlink(callback) {
    ...
    connection.query(`SELECT url FROM URLTABLE WHERE shortlink = $1`, [shortlink], function (err, results) {
        if (results && results.rowCount > 0) {
            generateshortlink(callback); // retry on collision
        } else {
            callback(shortlink);
        }
    });
...
app.get('/:s', (req, res) => {
    ...
    connection.query(`SELECT url FROM URLTABLE WHERE shortlink = $1`, [shortlink], (err, results) => {
        if (err || results.rowCount === 0) return res.sendStatus(404);
...
app.get('/give-url', (req, res) => {
    if (!globalUrl) return res.status(404).json({ error: 'No URL found' });
